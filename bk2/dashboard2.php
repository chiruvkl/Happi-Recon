...
/* $trans_type = (isset($_REQUEST['trans_type']) && $_REQUEST['trans_type'] != '')?intval(trim($_REQUEST['trans_type'])):'';
$store_name = (isset($_REQUEST['store_name']) && $_REQUEST['store_name'] != '')?trim($_REQUEST['store_name']):'';
$get_date = (isset($_REQUEST['get_date']) && $_REQUEST['get_date'] != '')?trim($_REQUEST['get_date']):''; */



$u_query = "SELECT * FROM happi_all_transactions where acquirer IN ('SBI87', 'HDFC') ORDER BY id DESC";

//echo $user_type.'-'.$u_query;
$u_res = mysqli_query($conn, $u_query);
$u_count = mysqli_num_rows($u_res);

$acquirer_query = "SELECT store_name FROM happi_all_transactions GROUP BY store_name ORDER BY id ASC";  
					...
					<table class="table table-bordered" id="zero_config">
					  ...
					  while($mp_row22 = mysqli_fetch_assoc($u_res)){
						...
						if($acquirer == 'HDFC'){
							...
							$u_query1 = "SELECT * FROM happi_hdfc_transactions where approve_code = '$approval_code' ORDER BY id DESC LIMIT 0,1";
							
							//echo $user_type.'-'.$u_query;
							$u_res1 = mysqli_query($conn, $u_query1);
							$bankdata = mysqli_fetch_assoc($u_res1);
							$txn_amount = $bankdata['domestic_amt'];
							
						} else{
							$approval_code1 = str_replace("'", "", $approval_code);
							
							$u_query = "SELECT * FROM happi_sbi_transactions where auth_code = '$approval_code1' ORDER BY id DESC LIMIT 0,1"; 
							$u_res = mysqli_query($conn, $u_query);
							$bankdata = mysqli_fetch_assoc($u_res);
							$txn_amount = $bankdata['txn_amount'];
						} 
						  ...
						  <td><div class="form-group col-md-12"><strong><?php echo $mp_row22['tid'];?></strong></div></td>
						  <td><div class="form-group col-md-12"><strong><?php echo $mp_row22['payment_mode'];?></strong></div></td>
						  <td><div class="form-group col-md-12"><strong><?php echo $mp_row22['transaction_id'];?></strong></div></td>
						  <td><div class="form-group col-md-12"><strong><?php echo $mp_row22['approval_code'];?></strong></div></td>
						  <td><div class="form-group col-md-12"><strong><?php echo $mp_row22['amount'];?></strong></div></td>
						  ...
						  <td><div class="form-group col-md-12"><strong><?php echo $txn_amount;?></strong></div></td>
