/** 
* @preserve CanvasJS Chart - v3.7.11 GA - https://canvasjs.com/ 
...
var convertNumberToString = function (x) {
	var isNegative = x < 0;
	if (Math.abs(x) < 1.0) {
...
var numberFormat = function (v, fs, cultureInfo) {
	...
	if (isScientificNotation) {
		...
		exponent = v === 0 ? 0 : integer === 0 ? -(noPhBeforeDecimal + noOfZerosAfterDecimal) : convertNumberToString(integer).length - noPhBeforeDecimal;
...
function setCanvasSize(canvas, width, height) {
	...
	if (isCanvasSupported && !!optimizeForHiDPI) {
		...
		if (!isNullOrUndefined(arguments[3]) && arguments[3])
			ctx = canvas.getContext("2d", { willReadFrequently: true }); //For multiple read-back operations - Passing this will remove warning thrown in chromium based browsers
		else
...
function createCanvas(width, height) {
	...
	setCanvasSize(canvas, width, height, arguments[2]);
...
var CanvasJSChart = CanvasJS.Chart = (function () {
	...
	Chart.prototype._updateOptions = function () {
		...
		if (this.options.zoomEnabled || this.panEnabled) { //panEnabled will be set to true incase of stockchart
			if (this._zoomButton) {
				...
				setButtonState(this, this._zoomButton, "zoom");
			}
			else {
				var preventMouseEvent = false;
	...
	Chart.prototype._processMultiseriesPlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataSeries.axisPlacement === "normal" || dataSeries.axisPlacement === "xySwapped") {
				...
				var plotAreaXMin = plotUnit.axisX.sessionVariables.newViewportMinimum ? plotUnit.axisX.sessionVariables.newViewportMinimum : (plotUnit.axisX.options && plotUnit.axisX.options.viewportMinimum) ?
					plotUnit.axisX.options.viewportMinimum : (plotUnit.axisX.options && plotUnit.axisX.options.minimum) ? plotUnit.axisX.options.minimum : plotUnit.axisX.logarithmic ? 0 : -Infinity;

				var plotAreaXMax = plotUnit.axisX.sessionVariables.newViewportMaximum ? plotUnit.axisX.sessionVariables.newViewportMaximum : (plotUnit.axisX.options && plotUnit.axisX.options.viewportMaximum) ?
					plotUnit.axisX.options.viewportMaximum : (plotUnit.axisX.options && plotUnit.axisX.options.maximum) ? plotUnit.axisX.options.maximum : Infinity;
			}
	...
	Chart.prototype._processStackedPlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataSeries.axisPlacement === "normal" || dataSeries.axisPlacement === "xySwapped") {
				...
				var plotAreaXMin = plotUnit.axisX.sessionVariables.newViewportMinimum ? plotUnit.axisX.sessionVariables.newViewportMinimum : (plotUnit.axisX.options && plotUnit.axisX.options.viewportMinimum) ?
					plotUnit.axisX.options.viewportMinimum : (plotUnit.axisX.options && plotUnit.axisX.options.minimum) ? plotUnit.axisX.options.minimum : -Infinity;
	...
	Chart.prototype.replaceKeywordsWithValue = function (str, dp, ds, dpIndex, indexKeywordValue) {
		...
		var fcn = function ($0) {
			...
			if (key === "x") {
				if ((ds.axisX && ds.axisX.valueType === "dateTime" || ds.xValueType === "dateTime" || (dp.x && dp.x.getTime)) && !ds.axisX.logarithmic) {
	...
	DataSeries.prototype.getMarkerProperties = function (index, x, y, ctx) {
		...
		var markerBorderThickness = !isNullOrUndefined(dataPoints[index].markerBorderThickness) ? dataPoints[index].markerBorderThickness : dataSeries.markerBorderThickness ? dataSeries.markerBorderThickness : null;
		var markerType = dataPoints[index].markerType ? dataPoints[index].markerType : dataSeries.markerType;
		var markerSize = !isNullOrUndefined(dataPoints[index].markerSize) ? dataPoints[index].markerSize : dataSeries.markerSize;
			...
			while (i < 4) {
				...
				if (axisX && axisX.length > 0) {
					for (var k = 0; k < axisX.length; k++) {
						if (axisX[k]) {
							...
							axisX[k].bounds = {
								x1: x1, y1: y1, x2: x2, y2: y2 - (axisXTotalHeight + axisXMargin - axisXHeight[k] - prevAxisXHeight), width: x2 - x1
							};
							axisX[k].bounds.height = axisX[k].bounds.y2 - axisX[k].bounds.y1;
						}
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						//axisX.lineCoordinates = { x1: x1, y1: y1, x2: x2, y2: y1, width: Math.abs(x2 - x1) }
						y2 = axisX2[k].lineCoordinates.y1 = y1 + axisX2TotalHeight + axisX2Margin - prevAxisX2Height;
						...
						axisX2[k].bounds = {
							x1: x1, y1: y1 + (axisX2TotalHeight + axisX2Margin - axisX2Height[k] - prevAxisX2Height), x2: x2, y2: y2, width: x2 - x1
						};
						axisX2[k].bounds.height = axisX2[k].bounds.y2 - axisX2[k].bounds.y1;
				...
				if (axisY && axisY.length > 0) {
					for (var k = 0; k < axisY.length; k++) {
						...
						if (axisY[k]) {
							...
							axisY[k].bounds = {
								x1: x1 - (axisYWidth[k] + prevAxisYWidth), y1: y1, x2: x2 - prevAxisYWidth, y2: y2, height: y2 - y1
							};
							axisY[k].bounds.width = axisY[k].bounds.x2 - axisY[k].bounds.x1;
				...
				if (axisY2 && axisY2.length > 0) {
					for (var k = 0; k < axisY2.length; k++) {
						...
						if (axisY2[k]) {
							...
							axisY2[k].bounds = {
								x1: x1 + prevAxisY2Width, y1: y1, x2: x2 + (axisY2Width[k] + prevAxisY2Width), y2: y2, height: y2 - y1
							};
							axisY2[k].bounds.width = axisY2[k].bounds.x2 - axisY2[k].bounds.x1;
			...
			while (i < 4) {
				...
				if (axisY2 && axisY2.length > 0) {
					for (var k = axisY2.length - 1; k >= 0; k--) {
						if (axisY2[k]) {
							...
							axisY2[k].bounds = {
								x1: x1, y1: y1 + (axisY2[k].margin + prevAxisY2Height), x2: x2, y2: y2, width: x2 - x1
							};
							axisY2[k].bounds.height = axisY2[k].bounds.y2 - axisY2[k].bounds.y1;
				...
				if (axisX && axisX.length > 0) {
					for (var k = 0; k < axisX.length; k++) {
						...
						axisX[k].bounds = {
							x1: x2 - (axisXWidth[k] + prevAxisXWidth), y1: y1, x2: x2 - prevAxisXWidth, y2: y2, height: y2 - y1
						};

						axisX[k].bounds.width = axisX[k].bounds.x2 - axisX[k].bounds.x1;
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						axisX2[k].bounds.width = axisX2[k].bounds.x2 - axisX2[k].bounds.x1;
	...
	Axis.render = function (axisX, axisX2, axisY, axisY2, axisPlacement) {
		...
		if (axisX && axisX.length)
			ctx.rect(5, axisX[0].bounds.y1, axisX[0].chart.width - 10, axisX[axisX.length - 1].bounds.y2);

		if (axisX2 && axisX2.length)
			ctx.rect(5, axisX2[axisX2.length - 1].bounds.y1, axisX2[0].chart.width - 10, axisX2[0].bounds.y2);
		...
		if (this.snapToDataPoint) {
			...
			if (this.chart.plotInfo.axisPlacement === "xySwapped") {
				...
				if (this.parent._position === "bottom" || this.parent._position === "top") {
					...
					else if (this.parent.valueType === "dateTime" && !this.parent.logarithmic) {
		...
		if (this.shared && this.enabled && this.chart.plotInfo.axisPlacement !== "none") {
			...
			if (this.chart.axisX) {
				for (var k = 0; k < this.chart.axisX.length; k++) {
					x = this.chart.plotInfo.axisPlacement === "xySwapped" ? this.chart.axisX[k].convertPixelToValue({ y: mouseY }) : this.chart.axisX[k].convertPixelToValue({ x: mouseX });
			...
			if (this.chart.axisX2) {
				for (var k = 0; k < this.chart.axisX2.length; k++) {
					x = this.chart.plotInfo.axisPlacement === "xySwapped" ? this.chart.axisX2[k].convertPixelToValue({ y: mouseY }) : this.chart.axisX2[k].convertPixelToValue({ x: mouseX });
	...
	ToolTip.prototype.showAtX = function (xValue, dataSeriesIndex) {
		...
		if (entries.length > 0) {
			for (var i = 0; i < entries.length; i++) {
				...
				if ((this.shared || entry.dataSeries.type.indexOf("100") >= 0) && (entry.dataPoint.x >= entry.dataSeries.axisX.viewportMinimum && entry.dataPoint.x <= entry.dataSeries.axisX.viewportMaximum)) {
					valueOutsideViewport = false;
					break;
				}
				else if (entry.dataPoint.x < entry.dataSeries.axisX.viewportMinimum || entry.dataPoint.x > entry.dataSeries.axisX.viewportMaximum || entry.dataPoint.y < entry.dataSeries.axisY.viewportMinimum || entry.dataPoint.y > entry.dataSeries.axisY.viewportMaximum) {
	...
	function EventManager(chart) {
		...
		this.ghostCanvas = createCanvas(this.chart.width, this.chart.height, true);
...
CanvasJSChart.version = "v3.7.11 GA";
