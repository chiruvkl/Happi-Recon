			...
			if(is_uploaded_file($_FILES['csvfile2']['tmp_name'])){
				...
				while(($line = fgetcsv($csvFile)) !== FALSE){
					...
					if($credit_amount != '' && $description != ''&& $value_date != ''){			
						...
						$utr_no_dec = $description_val[4];
						
						$sql = "SELECT id FROM happi_hdb_kvb_transactions WHERE credit_amount = '$credit_amount' AND description = '$description' AND value_date = '$value_date'";				
						$qry = mysqli_query($conn, $sql);
						$res  = mysqli_fetch_assoc($qry);	
						
						if($res['id'] == ''){
					
							$qry = "INSERT INTO happi_tvs_kvb_transactions ( credit_amount, 
								description, 
								value_date, 
								utr_no_dec 
							) VALUES (
