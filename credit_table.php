...
$phonepay_total_query = "SELECT COUNT(*) AS total_rows, SUM(ph_amount) AS phonepay_total , SUM(fee) AS fee 
						FROM happi_phonepe_transactions WHERE transaction_date = '$get_date'";
...
if($phonepay_total_res){
	...
	$phonepay_fee = $phonepay_total_assoc['fee'];
	$phonepay_total_cnt = $phonepay_total_assoc['total_rows'];
}

$other_total_query = "SELECT COUNT(*) AS total_rows, SUM(amount) AS other_total 
					FROM happi_all_transactions WHERE datetime = '$get_date' AND acquirer NOT IN ('PHONEPE','HDFC','SBI87')";
...
$sbi_matched_query = "SELECT a.id FROM happi_sbi_transactions a JOIN (
						SELECT approval_code,datetime, MIN(id) AS id
						FROM happi_all_transactions
						WHERE acquirer = 'SBI87'
						GROUP BY approval_code,datetime
					) b ON (b.approval_code = a.auth_code AND b.datetime = a.tran_date)
					JOIN (
						SELECT apr_no,trn_date, MIN(id) AS id
						FROM happi_apx_transactions
						GROUP BY apr_no,trn_date
					) c ON (c.apr_no = a.auth_code AND c.trn_date = a.tran_date)
					WHERE a.tran_date = '$get_date'
					ORDER BY a.id ASC";
/* $sbi_matched_query = "SELECT a.id FROM happi_sbi_transactions a JOIN happi_all_transactions b ON b.approval_code = a.auth_code AND b.acquirer = 'SBI87'
					JOIN happi_apx_transactions c ON c.apr_no = a.auth_code WHERE a.tran_date = '$get_date' ORDER BY a.id ASC";	 */			
...
$hdfc_matched_query = "SELECT a.id FROM happi_hdfc_transactions a JOIN (
						...
						WHERE acquirer = 'HDFC'
						GROUP BY approval_code,datetime
					) b ON (b.approval_code = a.approve_code AND b.datetime = a.trans_date)
					JOIN (
						SELECT apr_no,trn_date, MIN(id) AS id
						FROM happi_apx_transactions
						GROUP BY apr_no,trn_date
					) c ON (c.apr_no = a.approve_code AND c.trn_date = a.trans_date)
...
/* $phonepay_matched_query = "SELECT a.id
				...
				JOIN happi_all_transactions d ON(u.transaction_id = d.transaction_id AND d.datetime = a.transaction_date) 
				where a.transaction_date = '$get_date' ORDER BY a.id ASC"; //echo $phonepay_matched_query; */
$phonepay_matched_query = "SELECT a.id
				FROM happi_phonepe_transactions a 
				JOIN happi_upi_transactions u ON(u.host_txn_id = a.phonepe_referenceId AND u.txn_date = a.transaction_date) 
				JOIN happi_apx_transactions c ON(c.card_no = u.bill_reference_no AND c.trn_date = a.transaction_date) 
...
$other_matched_query = "SELECT a.id
				...
				JOIN happi_apx_transactions c ON(c.card_no = a.transaction_id AND c.trn_date = a.datetime) 
...
$total_matched_cnt = $sbi_matched_count + $hdfc_matched_count + $phonepay_matched_count + $other_matched_count;
if($pine_total_cnt != 0){
	$total_matched_cnt_pine = $sbi_matched_count + $hdfc_matched_count + $phonepay_matched_count + $other_matched_count;
}else{
	$total_matched_cnt_pine = 0;
}
					 ...
					 <table border="1" class="table result-table" cellspacing="0" style=" text-align: center;width:100%;">
						...
						<tbody style="background-color: #d3d3d3;">
							<tr>
								...
								<td><strong><a href="dwd_pine.php?get_date=<?php echo $get_date;?>"><?php echo $total_matched_cnt_pine;?></a></strong></td>
								<td><strong><a href="dwd_notm_pine.php?get_date=<?php echo $get_date;?>"><?php echo $pine_total_cnt - $total_matched_cnt_pine;?></a></strong></td>
							...
							<tr>
								...
								<td><a href="dwd_notm_phonepay.php?get_date=<?php echo $get_date;?>"><?php echo $phonepay_total_cnt - $phonepay_matched_count;?></a></td>
								<td><?php echo formatIndianCurrency($phonepay_total);?></td>
								<td><?php echo formatIndianCurrency($phonepay_fee);?></td>
							...
							<tr>
								...
								<td><a href="dwd_notm_other.php?get_date=<?php echo $get_date;?>"><?php echo $other_total_cnt - $other_matched_count;?></a></td>
								<td><?php echo formatIndianCurrency($other_total);?></td>
								<td>0.00</td>
							</tr>
							<tr>
								<td><strong>Apx PINE LABS-POS</strong></td>
								<td><strong><?php echo $apx_total_cnt;?></strong></td>
								<td><strong><a href="dwd_apx.php?get_date=<?php echo $get_date;?>"><?php echo $total_matched_cnt;?></a></strong></td>
								<td><strong><a href="dwd_notm_apx.php?get_date=<?php echo $get_date;?>"><?php echo ($apx_total_cnt - $total_matched_cnt);?></a></strong></td>
