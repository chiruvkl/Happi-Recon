			...
			if(is_uploaded_file($_FILES['csvfile2']['tmp_name'])){
				...
				while(($line = fgetcsv($csvFile)) !== FALSE){
					...
					if($voucher_no != '' && $apr_no != '' && $invoice_no != ''){					
						...
						$sql = "SELECT id FROM happi_apx_transactions WHERE apr_no = '$apr_no' AND trn_date = '$trn_date' AND invoice_no = '$invoice_no'  AND voucher_no = '$voucher_no' ";				
						$qry = mysqli_query($conn, $sql);
						$res  = mysqli_fetch_assoc($qry);	
						if($i > 3){		
							if($res['id'] == ''){
						
								$qry = "INSERT INTO happi_apx_transactions (voucher_no,card_no,apr_no,invoice_no, amount, trn_date, created_on) 
								VALUES ('$voucher_no', '$card_no','$apr_no', '$invoice_no', '$amount','$trn_date','$timestamp')";
